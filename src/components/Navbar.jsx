  ...
  return (
    <motion.header
      className={`sticky top-0 z-30 border-b backdrop-blur-xl transition-all duration-300 ${
        scrolled
          ? theme === "dark"
            ? "border-white/20 bg-slate-950/90 shadow-lg shadow-indigo-500/10"
    ...
    >
      <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
        ...
        {/* Desktop Nav */}
        <nav
          className={`hidden md:flex items-center gap-6 text-sm font-medium ${theme === "dark" ? "text-slate-200" : "text-slate-700"}`}
        >
        ...
        {open && (
          <div
            className={`absolute left-0 right-0 top-full md:hidden backdrop-blur-md border-t px-6 py-4 ${theme === "dark" ? "bg-black/80 border-white/10" : "bg-white/90 border-black/10"}`}
          >
