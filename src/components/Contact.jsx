  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      setStatus({
        loading: false,
        ok: res.ok,
        message: data?.message || (res.ok ? "Message sent" : "Failed to send"),
      });
    ...
    <section id="contact" className="mx-auto max-w-6xl px-6 py-20">
      ...
      >
        <span className="text-indigo-400 font-semibold">
          Open to internship opportunities and entry-level positions
        </span>{" "}
        in Android Development, Machine Learning enthusiast.
        <br />
        <br />
        If you're looking for a dedicated developer who is passionate about
        building scalable applications and solving complex problems, let's
        connect. I'm excited to contribute to innovative teams and grow
        professionally.
      </motion.p>
      ...
      >
        <h3
          className={`text-sm font-semibold mb-4 ${theme === "dark" ? "text-slate-300" : "text-slate-700"}`}
        >
          Connect with me
        </h3>
        <div className="flex items-center gap-3">
          {[
            {
              icon: Linkedin,
              href: "https://www.linkedin.com/in/niraj-kumar-31b34b28b/",
              label: "LinkedIn",
              color: "text-blue-400",
            },
            {
              icon: Github,
              href: "https://github.com/Niraj-patel62",
              label: "GitHub",
              color: "text-slate-400",
            },
            {
              icon: Instagram,
              href: "https://www.instagram.com/niraj__patel_174",
              label: "Instagram",
              color: "text-pink-400",
            },
          ].map((social) => (
      ...
      >
        <input
          ...
          className={`w-full rounded-md px-4 py-3 border focus:outline-none focus:ring-2 focus:ring-indigo-500 ${theme === "dark" ? "border-white/20 bg-white/5 text-slate-200 placeholder:text-slate-500" : "border-slate-300 bg-white text-slate-800 placeholder:text-slate-400"}`}
        />
        <input
          name="email"
          type="email"
        ...
        <input
          name="subject"
          placeholder="Subject"
          className={`w-full md:col-span-2 rounded-md px-4 py-3 border focus:outline-none focus:ring-2 focus:ring-indigo-500 ${theme === "dark" ? "border-white/20 bg-white/5 text-slate-200 placeholder:text-slate-500" : "border-slate-300 bg-white text-slate-800 placeholder:text-slate-400"}`}
        />
        <textarea
          name="message"
          required
          rows={5}
          placeholder="Message"
          className={`w-full md:col-span-2 rounded-md px-4 py-3 border resize-none focus:outline-none focus:ring-2 focus:ring-indigo-500 ${theme === "dark" ? "border-white/20 bg-white/5 text-slate-200 placeholder:text-slate-500" : "border-slate-300 bg-white text-slate-800 placeholder:text-slate-400"}`}
        />
        <button
